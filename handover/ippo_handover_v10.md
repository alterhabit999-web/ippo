...
| アイコンデザイン | 太極図（YANG:オレンジ×YIN:スチールブルー）・ソフトカラー | 完了 |
...
| `src/index.js` | Reactエントリポイント + Service Worker登録（PWA） |
| `public/index.html` | タイトル「iPPO」・ビューポート・PWA metaタグ・apple-touch-icon |
| `public/favicon.ico` | ブラウザタブアイコン（太極図・32px） |
| `public/logo192.png` | PWAアイコン（太極図・192px） |
| `public/logo512.png` | PWAアイコン（太極図・512px） |
| `public/apple-touch-icon.png` | iOSホーム画面アイコン（太極図・180px） |
| `public/manifest.json` | PWA設定（アプリ名・アイコン・standalone・theme_color） |
| `public/sw.js` | Service Worker（ネットワーク優先・Supabase除外・オフライン対応） |
...
### STEP 1：`npm run deploy` でデプロイ（未実施の場合）
- 最新のアイコン・PWA対応を本番に反映させる

### STEP 2：データを蓄積させる
- しばらく自分・家族で使い続けてデータを集める
- インサイト画面の分析をより意味のあるものにするため

### STEP 3：React Native（Expo）化
...
> 「iPPOの開発を続けます。引き継ぎ書v10と仕様書v1.8を確認してください。」
...
| 10.0 | 2026-04-10 | ブラウザタイトル変更・記録後遷移修正・言葉追加ウィンドウ統一・quotesテーブルDB化・みんなの言葉機能・複数ユーザー対応（UNIQUE制約変更）・デフォルト言葉管理・重複フィルター・ESLintワーニング修正・PWA対応（manifest・sw.js・太極図アイコン・iOS metaタグ） |
