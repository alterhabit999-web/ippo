...
| カレンダーストリーク表示 | カレンダー画面の凡例下に連続記録日数を表示 | 完了 |
| リマインド通知機能 | 設定時刻にWeb Notification APIでブラウザ通知を発行 | 完了 |
...
| `public/sw.js` | Service Worker v2（ネットワーク優先・HTML常時最新・Supabase除外・通知クリック対応） |
...
### ⚠️ デプロイ時の注意（重要）
- `npm run deploy` は必ず `.env` ファイルがあるディレクトリから実行すること
- `.env` がない状態でビルドすると Supabase 接続情報が `undefined` になりホワイトアウトする
- Coworkのworktreeから作業した場合、`.env` が存在しないことがあるので確認すること
- 確認コマンド：`ls ~/Documents/App/ippo/.env`
...
### カレンダー画面にストリーク表示を追加
**内容**: カレンダー画面の凡例（朝の記録あり・夜の記録あり）の下に、連続記録日数を表示
**詳細**:
- 今日から遡って連続で記録がある日数を計算（朝または夜どちらかの記録があればカウント）
- 「X日連続記録中」という数字＋テキスト横並びのピルデザインで表示
- 連続記録が途切れている（記録0日）場合は非表示

### リマインド通知機能を実装
**内容**: 設定画面で設定したリマインダー時刻にブラウザのプッシュ通知を発行
**詳細**:
- 設定値（ON/OFF・時刻）を `localStorage` に永続化（アプリを閉じても設定が保持される）
- トグルON時に `Notification.requestPermission()` を呼び出し、通知許可をリクエスト
- App component の `useEffect` 内で30秒ごとに時刻チェック → 設定時刻と一致したらWeb Notification APIで通知
- 同日中に同じ通知（朝・夜）を2回発行しないよう `localStorage` でガード
- 通知が拒否されている状態でトグルをONにすると警告メッセージを設定画面内に表示
- `public/sw.js` に `notificationclick` イベントを追加 → 通知タップでアプリ画面にフォーカス・起動

### 【バグ修正】デプロイ後にホワイトアウトが発生
**問題**: `npm run deploy` 後に本番ページが真っ白（ホワイトアウト）になった
**原因**: Coworkのworktreeディレクトリ（`.env` ファイルが存在しない場所）からビルド・デプロイしたため、`REACT_APP_SUPABASE_URL` 等の環境変数が `undefined` のままビルドに埋め込まれ、Supabase接続に失敗した
**修正内容**: `.env` をworktreeにコピーして再デプロイ。今後は必ず `.env` の存在を確認してからデプロイすること（セクション6の注意事項に追記）

---

## 8. PWAホーム画面追加手順（iPhone）
...
### STEP 1：試験運用（現在）
- しばらく自分・家族で使い続けてデータを蓄積させる
- 通知機能・ストリーク表示の使用感を確認する
- インサイト画面の分析をより意味のあるものにするため、データを集める
...
- プッシュ通知の本実装（現在はWeb Notification APIでの暫定対応）
- オフライン対応
...
- [ ] ホーム画面ウィジェット（iOS 14+）
- [x] ストリーク（連続記録日数）表示 ← 実装済み（カレンダー画面）
- [ ] AIによる週次インサイト生成（Claude API活用）
- [ ] 週次・月次レポートのPDF/画像エクスポート
- [ ] HAPPY/モヤモヤのキーワード分析（インサイト画面）
- [ ] カスタムテーマ（プレミアム機能候補）
- [ ] Apple Health 連携
- [x] 通知機能 ← Web Notification APIで暫定実装済み（React Native化後に本実装）
...
> 「iPPOの開発を続けます。引き継ぎ書v12と仕様書v2.0を確認してください。」
...
| 12.0 | 2026-04-14 | カレンダー画面にストリーク表示追加・リマインド通知機能実装（Web Notification API）・デプロイ時の.env注意事項を追加 |
