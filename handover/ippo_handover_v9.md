...
| `public/index.html` | ブラウザタブタイトル・ビューポート設定済み |
| `public/favicon.ico` | ブラウザタブアイコン（現在デフォルト） |
| `public/manifest.json` | PWA設定（現在デフォルト） |
...
### STEP 1：ブラウザタイトルを「iPPO」に変更（5分）
- `public/index.html` の `<title>` タグを `iPPO` に変更
- デプロイするだけで即反映

### STEP 2：ファビコン・アイコン設定（30〜60分）
- Canva等でiPPO用アイコン画像を作成（正方形）
- `favicon.ico`（32px）、`logo192.png`（192px）、`logo512.png`（512px）、`apple-touch-icon.png`（180px）を作成
- `public/manifest.json` のアプリ名を更新

### STEP 3a：PWA対応（ホーム画面追加）（1〜2時間）
- `manifest.json` 整備 + Service Worker追加
- iPhoneのSafariから「ホーム画面に追加」で疑似アプリ化
- コードの大幅書き直し不要・工数小

### STEP 3b：React Native（Expo）化（長期・大工数）
- 通知機能・App Store / Google Play申請のため
- Expo使用でWebアプリとのコード共有率を高める
- Apple Developer Program（年99ドル）が必要

**推奨移行順序**：
```
現在（GitHub Pages Webアプリ）
  ↓ STEP 1+2
ブラウザタイトル・アイコン設定（完成度UP）
  ↓ STEP 3a
PWA対応（ホーム画面アイコン化）← まずここ
  ↓ STEP 3b
Expo（React Native）化
  ↓
...
| 9.0 | 2026-04-09 | あつめた言葉たち画面・言葉をあつめるボタン・iPPOタイトルナビゲーション・サイドメニュー順序・カレンダー詳細パネル改善・次回開発計画（ブラウザタイトル・ファビコン・スマホアプリ移行）追加 |
